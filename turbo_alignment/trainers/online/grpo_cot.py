...
class GRPOTrainer(MultiGPUCherryPicksTrainer):
    ...
    def get_answers_and_rewards(
        ...
        answer_start_ids = self.tokenizer.encode('<answer>', add_special_token=False)
        answer_end_ids = self.tokenizer.encode('</answer><|im_end|>', add_special_token=False)

        rewards = []

        for idx, msg_ans in enumerate(message_answers):
            ...
            if torch.distributed.get_rank() == 0:
                # print('input_, ', input_)
                print('before response_ids[idx].shape, ', response_ids[idx].shape)
                print('before response_attention_mask[idx].shape, ', response_attention_mask[idx].shape)
            
            torch.distributed.barrier()

            rewards.append(self.critic_generator[input_])

            gt_tokens = self.tokenizer.encode(self.critic_generator[input_], add_special_tokens=False)

            response_ids[idx] = torch.cat([response_ids[idx], torch.tensor(answer_start_ids + gt_tokens + answer_end_ids, device=response_ids[idx].device).unsqueeze(0)], dim=1) # check true concat
            response_attention_mask[idx] = torch.cat(
                [
                    response_attention_mask[idx], 
                    torch.ones(len(answer_start_ids), device=response_attention_mask[idx].device).unsqueeze(0), 
                    torch.ones(len(gt_tokens), device=response_attention_mask[idx].device).unsqueeze(0), 
                    torch.ones(len(answer_end_ids), device=response_attention_mask[idx].device).unsqueeze(0),
                ], 
                dim=1,
            )
            ...
            if torch.distributed.get_rank() == 0:
                # print('input_, ', input_)
                print('after response_ids[idx].shape, ', response_ids[idx].shape)
                print('after response_attention_mask[idx].shape, ', response_attention_mask[idx].shape)
    ...
    def get_batch_loss_metrics(
        ...
        rewards = logprobs[:, -1].detach()

        start_cot_token_id = self.tokenizer.encode('<think>', add_special_tokens=False)[0]
        end_cot_token_id = self.tokenizer.encode('</think>', add_special_tokens=False)[0]

        cot_loss_mask = torch.zeros_like(query_response, dtype=torch.bool, device=query_response.device)

        for b in range(query_response.size(0)):
            tokens = query_response[b]
            
            start_positions = (tokens == start_cot_token_id).nonzero(as_tuple=True)
            end_positions   = (tokens == end_cot_token_id).nonzero(as_tuple=True)

            if len(start_positions) != 0 and end_positions != 0:
                cot_loss_mask[b, start_positions[0][0].item(): end_positions[0][0].item() + 1] = True
            else:
                cot_loss_mask[b, 0: -1] = False
                rewards[b, :] = self.args.penalty_reward_value

        if torch.distributed.get_rank() == 0:
            end = time.time()
            self.time_profiler.policy_model_time.append(end - start)

        cot_loss_mask = cot_loss_mask[:, 1:]

        answer_loss_mask = torch.logical_xor(response_tokens_mask, cot_loss_mask)

        # ref_logprobs = (ref_logprobs * cot_loss_mask).sum(-1)
        # logprobs = (logprobs * cot_loss_mask).sum(-1)
        ...
        cot_per_token_loss = torch.exp(logprobs - logprobs.detach()) * advantages.unsqueeze(1) * cot_loss_mask
        answer_per_token_loss = logprobs * answer_loss_mask
        per_token_loss = -((cot_per_token_loss  + answer_per_token_loss) - self.kl_coef * kl_term)
