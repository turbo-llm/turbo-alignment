...
class BlockWithMasking(torch.nn.Module):
    def __init__(
        ...
        drop_path: float = 0.0,
        layer_scale_type: str | None = None,
        layer_scale_init_value: float = 1e-4,
    ):
        super().__init__()

        assert not isinstance(
            attn_target, torch.nn.Module
        ...
        if self.layer_scale_type is not None:
            ...
            self.layer_scale_gamma1 = torch.nn.Parameter(
                torch.ones(size=gamma_shape) * layer_scale_init_value,
                requires_grad=True,
            )
            self.layer_scale_gamma2 = torch.nn.Parameter(
                torch.ones(size=gamma_shape) * layer_scale_init_value,
                requires_grad=True,
            )
...
class SimpleTransformer(torch.nn.Module):
    def __init__(
        ...
        pre_transformer_layer: Callable | None = None,
        post_transformer_layer: Callable | None = None,
        drop_path_rate: float = 0.0,
        drop_path_type: str = 'progressive',
        norm_layer: Callable = partial(torch.nn.LayerNorm, eps=1e-6),
        mlp_ratio: int = 4,
        ffn_dropout_rate: float = 0.0,
        layer_scale_type: str | None = None,  # from cait; possible values are None, "per_channel", "scalar"
    ...
    def forward(
        ...
        checkpoint_blk_ids: list[int] | None = None,
    ):
        """
        Inputs
        ...
        for blk_id, blk in enumerate(self.blocks):
            if use_checkpoint and checkpoint_blk_ids is not None and blk_id in checkpoint_blk_ids:
