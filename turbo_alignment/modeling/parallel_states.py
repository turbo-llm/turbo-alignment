...
_DATA_PARALLEL_ALL_GROUP_RANKS = None
...
def initialize_model_parallel(
    ...
    for i in range(num_pipeline_model_parallel_groups):
        ...
        if len(ranks) > 1:
            ...
            #         embedding_ranks = [
            #             ranks[0], ranks[pipeline_model_parallel_split_rank], ranks[-1]]
...
def sequence_parallel_is_enabled():
    return model_parallel_is_initialized() and get_sequence_parallel_world_size() > 1


def get_sequence_parallel_world_size_or_one():
    return get_sequence_parallel_world_size() if model_parallel_is_initialized() else 1
...
def get_sequence_parallel_world_size_or_none():
...
def get_all_data_parallel_global_ranks():
    assert _DATA_PARALLEL_ALL_GROUP_RANKS is not None, "Data parallel group is not initialized"
    return _DATA_PARALLEL_ALL_GROUP_RANKS
