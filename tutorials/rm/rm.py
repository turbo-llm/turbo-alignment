...
if __name__ == '__main__':
    ...
    ds['train_pair_preference_records'] = ds['train_prefs'].map(
        convert_to_preference_record, remove_columns=ds['train_prefs'].column_names
    )
    ds['val_pair_preference_records'] = ds['test_prefs'].map(
        convert_to_preference_record, remove_columns=ds['test_prefs'].column_names
    )
    ...
    launch_code = (
        'poetry run python -m turbo_alignment train_rm --experiment_settings_path turbo-alignment/tutorials/rm/rm.json'
    )
